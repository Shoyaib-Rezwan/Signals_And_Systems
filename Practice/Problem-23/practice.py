...
def cross_correlation_via_dft(x:np.ndarray, h:np.ndarray)->np.ndarray:
    ...
    return dftAnalyzer.compute_idft(X*np.conj(Y))[:N]
...
def reconstruct_image_using_fft(original_path, shifted_path, output_path):
    ...
    reconstructed_img=np.copy(shift_gray)
    row_size, col_size=np.shape(reconstructed_img)
    print(row_size, col_size)
    for k in range(row_size):
        shift_amt=np.argmax(np.real(cross_correlation_via_dft(orig_gray[k,:], reconstructed_img[k,:])))
