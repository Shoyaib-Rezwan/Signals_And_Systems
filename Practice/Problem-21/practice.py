...
row_idx=np.argmin(np.min(encrypted_image,axis=1))
h=encrypted_image[row_idx,:]
row_no,col_no=np.shape(decrypted_image)

for i in range(row_no):
    if i==row_idx:
        continue
    decrypted_image[i,:]=np.real(reverse_circular_convolution(decrypted_image[i,:],h))
