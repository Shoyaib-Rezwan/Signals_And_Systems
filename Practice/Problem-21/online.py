...
def circular_convolution_by_dft(x:np.ndarray,h:np.ndarray)->np.ndarray:
...
min_row_idx=np.argmin(np.min(encrypted_image,axis=1))
h=encrypted_image[min_row_idx,:]
print(h)
...
decrypted_image=np.zeros(np.shape(encrypted_image),dtype=np.float64)
k=0
for row in encrypted_image:
    if k==min_row_idx:
        decrypted_image[k,:]=row
    else:
        decrypted_image[k,:]=np.real(circular_convolution_by_dft(row,h))
    k+=1
