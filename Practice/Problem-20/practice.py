...
def circular_convolution(x:np.ndarray, y=np.ndarray)->np.ndarray:
    ...
    Y=dftAnalyzer.compute_dft(DiscreteSignal(y))
    return dftAnalyzer.compute_idft(X*Y)
...
x=x[::-1]
y=y[::-1]
totalLen=len(x)+len(y)-1
x=DiscreteSignal(x).pad(totalLen).data
y=DiscreteSignal(y).pad(totalLen).data
z=np.round(np.real(circular_convolution(np.array(x), np.array(y)))).astype(np.int64)
result=[]
carry=0
for n in z:
