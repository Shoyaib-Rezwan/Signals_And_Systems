...
coefficeients1=list(map(int,input().split()))
d2=int(input("Degree of the second polynomial: "))
coefficeients2=list(map(int,input().split()))
d=d1+d2
signal1=DiscreteSignal(d1)
for i in range(d1+1):
    signal1.set_value_at_time(i,coefficeients1[i])

signal2=DiscreteSignal(d2)
for i in range(d2+1):
    signal2.set_value_at_time(i,coefficeients2[i])

# signal2.plot()
# plt.show()

system=LTI_Discrete_class(signal1)
output_signal=system.output(signal2)
# output_signal.plot()
# plt.show()
coefficeients=output_signal.values[output_signal.INF:output_signal.INF+d+1]
print(f"Degree of the polynomial: {d}\nCoefficients: {coefficeients}")
