...
def signalScale(signal: np.ndarray, k:np.int8)->np.ndarray:
    result=np.zeros_like(signal)
    secondHalf=signal[INF::k]
    signal=np.flip(signal)
    firstHalf=signal[INF::k]
    firstHalf=np.flip(firstHalf)[:-1]
    result[INF:INF+secondHalf.size]=secondHalf
    result[INF-firstHalf.size:INF]=firstHalf
    return result
