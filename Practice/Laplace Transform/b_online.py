...
class SmartIrrigation:
    ...
    def mae(self,h):
        return np.trapezoid(np.abs(h-corruptSignal(h,self.t)))/self.t_max
    
    def pae(self,h):
        return np.max(np.abs(h-corruptSignal(h,self.t)))
...
def simulate_system(a=0.5,b=1.0):
    ...
    print(f"5. Number of sa esolution: Δt = π/W = {np.pi/W:.4f} s")
    ...
    for idx, (name, u) in enumerate(inputs.items()):
        ...
        # --- Euler ---
        u_corrupt=corruptSignal(u, system.t)
        h_corrupt = system.euler_simulate(u_corrupt)
        ...
        # Euler subplot
        axes[1].plot(system.t, u, 'b--', lw=1.8, label="Input u_corrupt(t)")
        axes[1].plot(system.t, h_corrupt, color='tomato', lw=2.2, label="Output h(t)")
        axes[1].set_title("Corrupted Signal Simulation", fontweight='bold')
        ...
        # plt.show()
def corruptSignal(u,t, Td=1.5, delta=0.2):
    u_quantized=delta*np.round(u/delta)
    shift=int(np.round(t[-1]/Td))
    u_corrupt=np.zeros_like(u)
    u_corrupt[shift:]=u_quantized[:len(t)-shift]
    return u_corrupt
