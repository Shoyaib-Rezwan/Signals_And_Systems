...
def simulate_system(a=0.5,b=1.0):
    ...
    for idx, (name, u) in enumerate(inputs.items()):
        ...
        # --- Euler ---
        u_error=corrupt_signal(u,system.t)
        h_error = system.euler_simulate(u_error)
        ...
        # Euler subplot
        axes[1].plot(system.t, u_error, 'b--', lw=1.8, label="Input u_corrupted(t)")
        axes[1].plot(system.t, h_error, color='tomato', lw=2.2, label="Output h_corrupted(t)")
        axes[1].set_title("Laplace Method Simulation for corrupted Signal", fontweight='bold')
        ...
        plt.show()
def corrupt_signal(u,t):
    return u-0.02*t+0.08*np.sin(t)
