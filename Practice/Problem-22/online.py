...
row_shift=np.argmax(np.real(cross_correlation_by_dft(image[best_row_idx,:],shifted_image[best_row_idx])))
if row_shift>=col_no//2:
    row_shift-=col_no
col_shift=np.argmax(np.real(cross_correlation_by_dft(image[:,best_col_idx],shifted_image[:,best_col_idx])))
if col_shift>=row_no//2:
    col_shift-=row_no

for k in range(row_no):
    reversed_shifted_image[k,:]=np.roll(reversed_shifted_image[k,:],row_shift)
for k in range(col_no):
    reversed_shifted_image[:,k]=np.roll(reversed_shifted_image[:,k],col_shift)
