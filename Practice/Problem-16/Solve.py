...
class ModifiedSignal(ContinuousSignal):
    ...
    y(t) = x(a*t) * e^(j2πf₀t)
    """
    def __init__(self, t, a, f0, base_signal):
        super().__init__(t)
        self.a = a
        self.f0 = f0
    ...
    def values(self):
        """Compute y(t) = x(a*t) * e^(j2πf₀t)"""
        x_values = self.base_signal.values()
        compressed_t = self.a * self.t
        x_compressed = np.interp(compressed_t, self.t, x_values, left=0.0, right=0.0)
        phase_shift = np.exp(1j * 2 * np.pi * self.f0 * self.t)
        return x_compressed * phase_shift
...
class ComplexCFTAnalyzer(CFTAnalyzer):
    ...
    def compute_cft(self):
        ...
        for i, freq in enumerate(self.frequencies):
            cos_term = np.cos(2 * np.pi * freq * self.t)
            sin_term = np.sin(2 * np.pi * freq * self.t)
            
            # Separate real and imaginary parts of signal
            signal_real = np.real(signal_values)
            signal_imag = np.imag(signal_values)
            
            # Real part: Re{signal} * cos + Im{signal} * sin
            real_integrand = signal_real * cos_term + signal_imag * sin_term
            real_part[i] = np.trapezoid(real_integrand, self.t)
            
            # Imaginary part: Im{signal} * cos - Re{signal} * sin
            imag_integrand = signal_imag * cos_term - signal_real * sin_term
            imaginary_part[i] = np.trapezoid(imag_integrand, self.t)
...
verifier = CFTVerifier(spectrum_x, spectrum_y, frequencies, a, f0)
verifier.verify()

print("\nInterpretation:")
print("(i)  Phase shift by e^(j2πf₀t) shifts spectrum to the right by f₀")
print("(ii) Time compression by a=10 expands and scales spectrum by 1/a")
print("Combined: Y(f) = (1/|a|) * X((f - f₀) / a)")
